...
public class UrsaAutoSettingPanel : TemplatedControl
{
    ...
    public static readonly StyledProperty<IPropertyValueAccessor?> PropertyAccessorProperty =
        AvaloniaProperty.Register<UrsaAutoSettingPanel, IPropertyValueAccessor?>(nameof(PropertyAccessor));
    ...
    public static readonly StyledProperty<ILocalizationService?> LocalizationServiceProperty =
        AvaloniaProperty.Register<UrsaAutoSettingPanel, ILocalizationService?>(nameof(LocalizationService));
    ...
    private global::Avalonia.Controls.Control CreateTextBoxWithFeatures(Core.Models.PropertyDescriptor prop, object target)
    {
        ...
        if (propertyInfo != null && ControlLayoutHelper.GetValidations(propertyInfo).Length > 0)
        {
            textBox.TextChanged += (s, e) =>
            {
                var newValue = textBox.Text;
                if (ControlLayoutHelper.ValidateAndUpdateVisual(textBox, newValue, propertyInfo, target))
                {
    ...
    private global::Avalonia.Controls.Control CreateCustomCollectionEditor(Core.Models.PropertyDescriptor prop, object target)
    {
        ...
        if (editor != null)
        {
            if (editor is global::Avalonia.Controls.Control control)
            {
                // Set the collection as DataContext for binding
                control.DataContext = _accessor?.GetValue(target, prop.PropertyName);
            }
            return editor;
        }
    ...
    private IDataTemplate CreateCollectionItemTemplate(Type elementType, bool allowEditItems)
    {
        return new FuncDataTemplate<object>((item, scope) =>
        {
            ...
            // Complex types
            var panel = new StackPanel { Spacing = 8, Margin = new Thickness(8), DataContext = item };
            ...
            foreach (var p in props)
            {
                ...
                row.ColumnDefinitions.Add(new ColumnDefinition(new GridLength(110)));
    ...
    {
        var min = prop.NumericMinimum;
        var max = prop.NumericMaximum;
        var inc = prop.NumericIncrement;

        switch (control)
        {
            case global::Ursa.Controls.NumericIntUpDown c:
                c.Minimum = (int)min;
                c.Maximum = (int)max;
                c.Step = (int)inc;
                break;
            case global::Ursa.Controls.NumericUIntUpDown c:
                c.Minimum = (uint)Math.Max(0, min);
                c.Maximum = (uint)Math.Max(0, max);
                c.Step = (uint)Math.Max(0, inc);
                break;
            case global::Ursa.Controls.NumericDoubleUpDown c:
                c.Minimum = min;
                c.Maximum = max;
                c.Step = inc;
                break;
            case global::Ursa.Controls.NumericFloatUpDown c:
                c.Minimum = (float)min;
                c.Maximum = (float)max;
                c.Step = (float)inc;
                break;
            case global::Ursa.Controls.NumericByteUpDown c:
                c.Minimum = (byte)Math.Max(0, min);
                c.Maximum = (byte)Math.Max(0, max);
                c.Step = (byte)Math.Max(0, inc);
                break;
            case global::Ursa.Controls.NumericSByteUpDown c:
                c.Minimum = (sbyte)min;
                c.Maximum = (sbyte)max;
                c.Step = (sbyte)inc;
                break;
            case global::Ursa.Controls.NumericShortUpDown c:
                c.Minimum = (short)min;
                c.Maximum = (short)max;
                c.Step = (short)inc;
                break;
            case global::Ursa.Controls.NumericUShortUpDown c:
                c.Minimum = (ushort)Math.Max(0, min);
                c.Maximum = (ushort)Math.Max(0, max);
                c.Step = (ushort)Math.Max(0, inc);
                break;
            case global::Ursa.Controls.NumericLongUpDown c:
                c.Minimum = (long)min;
                c.Maximum = (long)max;
                c.Step = (long)inc;
                break;
            case global::Ursa.Controls.NumericULongUpDown c:
                c.Minimum = (ulong)Math.Max(0, min);
                c.Maximum = (ulong)Math.Max(0, max);
                c.Step = (ulong)Math.Max(0, inc);
                break;
        }
    }
